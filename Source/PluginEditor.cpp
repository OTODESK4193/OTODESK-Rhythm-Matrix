    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    btnClearAll.onClick = [this] {
        juce::NativeMessageBox::showOkCancelBox(
            ...
            juce::ModalCallbackFunction::create([this](int result) {
                if (result == 1) {
                    audioProcessor.globalBarCount = 1; barCountMenu.setSelectedId(1, juce::dontSendNotification);
                    for (int i = 0; i < 8; ++i) { audioProcessor.clearTrack(i); audioProcessor.trackDivisions[i] = 4; divSelectors[i].setSelectedId(4, juce::dontSendNotification); audioProcessor.trackComplexity[i] = 50; complexitySliders[i].setValue(50.0, juce::dontSendNotification); }
    ...
    timeSigNumMenu.onChange = [this] { audioProcessor.timeSigNumerator.store(timeSigNumMenu.getSelectedId()); resized(); repaint(); };
    ...
    timeSigDenMenu.onChange = [this] {
        ...
        updateTimeSigNumMenu(); // ★ 分母変更時に分子の最大値を更新
        updateDivisionMenus();
        resized(); repaint();
        };
    ...
    barCountMenu.setSelectedId(audioProcessor.globalBarCount, juce::dontSendNotification);
    barCountMenu.onChange = [this] { audioProcessor.globalBarCount = barCountMenu.getSelectedId(); updateViewVisibility(); resized(); repaint(); };
    ...
    styleMenu.setSelectedId(1, juce::dontSendNotification);
    styleMenu.onChange = [this] { audioProcessor.currentGenre = styleMenu.getSelectedId() - 1; };
    ...
    for (int i = 0; i < 8; ++i) {
        addAndMakeVisible(trackNameLabels[i]); trackNameLabels[i].setText(defaultTrackNames[i], juce::dontSendNotification);
        ...
        btnClear[i].onClick = [this, i] { audioProcessor.clearTrack(i); repaint(); };
    ...
    generateButton.onClick = [safeThis, this] {
        ...
        if (genreIdx >= 0 && genreIdx < 24 && !audioProcessor.isSyncEnabled.load()) {
            juce::Random r; double newBpm = (double)r.nextInt(juce::Range<int>(minTempos[genreIdx], maxTempos[genreIdx] + 1));
            audioProcessor.internalTempo.store(newBpm);
        }
        for (int i = 0; i < 8; ++i) {
            audioProcessor.randomizeTrack(i);
            divSelectors[i].setSelectedId(audioProcessor.trackDivisions[i], juce::dontSendNotification);
            complexitySliders[i].setValue(audioProcessor.trackComplexity[i], juce::dontSendNotification);
        }
        currentViewBar = 0; updateTabColors(); resized(); repaint();
        };
...
void AIDrumMachineAudioProcessorEditor::updateDivisionMenus() {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (audioProcessor.trackDivisions[i] > maxDiv) {
            audioProcessor.trackDivisions[i] = maxDiv;
        }
        divSelectors[i].setSelectedId(audioProcessor.trackDivisions[i], juce::dontSendNotification);
    }
}
...
void AIDrumMachineAudioProcessorEditor::resized() {
    ...
    timeSigNumMenu.setBounds(row1.removeFromLeft(60).reduced(2)); // 50から60へ
    timeSigSlash.setBounds(row1.removeFromLeft(15));               // 10から15へ
    timeSigDenMenu.setBounds(row1.removeFromLeft(60).reduced(2)); // 50から60へ

    dragAllArea = row1.removeFromRight(120).toFloat();

    area.removeFromTop(10);
    auto row2 = area.removeFromTop(30);
    generateButton.setBounds(row2.removeFromLeft(120)); row2.removeFromLeft(10); styleMenu.setBounds(row2.removeFromLeft(200)); row2.removeFromLeft(10); statusLabel.setBounds(row2);
    ...
    tabButton1.setBounds(tabArea.removeFromLeft(tabW).reduced(2)); tabButton2.setBounds(tabArea.removeFromLeft(tabW).reduced(2)); tabButton3.setBounds(tabArea.removeFromLeft(tabW).reduced(2)); tabButton4.setBounds(tabArea.removeFromLeft(tabW).reduced(2));
    ...
    if (currentView == SequencerView) {
        ...
        auto labelArea = bottomArea.removeFromLeft(120); sampleArea = bottomArea.removeFromLeft(160); auto setupControls = bottomArea;
        int rows = 8; float cellH = sampleArea.getHeight() / rows;
        for (int row = 0; row < rows; ++row) {
            int idx = 7 - row;
            ...
            ctrlRow.removeFromLeft(20); divLabels[idx].setBounds(ctrlRow.removeFromLeft(30).toNearestInt()); divSelectors[idx].setBounds(ctrlRow.removeFromLeft(50).toNearestInt()); btnDivLock[idx].setBounds(ctrlRow.removeFromLeft(20).reduced(1).toNearestInt());
            ctrlRow.removeFromLeft(20); compLabels[idx].setBounds(ctrlRow.removeFromLeft(45).toNearestInt()); complexitySliders[idx].setBounds(ctrlRow.removeFromLeft(120).toNearestInt()); btnCmplxLock[idx].setBounds(ctrlRow.removeFromLeft(20).reduced(1).toNearestInt());
        }
...
void AIDrumMachineAudioProcessorEditor::paint(juce::Graphics& g) {
    ...
    if (currentView == SequencerView) {
        ...
        for (int row = 0; row < rows; ++row) {
            ...
            int div = audioProcessor.trackDivisions[idx];
            ...
            for (int col = 0; col < colsToDraw; ++col) {
                ...
                int vel = audioProcessor.drumPattern[idx][globalStep];
...
void AIDrumMachineAudioProcessorEditor::mouseDown(const juce::MouseEvent& e) {
    ...
    if (e.mods.isRightButtonDown() && sampleArea.contains(pos)) {
        ...
        if (idx >= 0 && audioProcessor.hasSampleLoaded(idx)) {
            juce::NativeMessageBox::showOkCancelBox(juce::MessageBoxIconType::WarningIcon, "Delete", "Delete sample?", this, juce::ModalCallbackFunction::create([this, idx](int r) { if (r == 1) { audioProcessor.clearSample(idx); trackNameLabels[idx].setText(defaultTrackNames[idx], juce::dontSendNotification); repaint(); } }));
        }
        return;
    }
    ...
    if (currentView == SequencerView) {
        ...
        // グリッドのクリック判定
            ...
            if (globalStep < 1024) { audioProcessor.drumPattern[idx][globalStep] = (audioProcessor.drumPattern[idx][globalStep] > 0) ? 0 : 100; repaint(); }
        }
    }
}
...
juce::File AIDrumMachineAudioProcessorEditor::exportMidi(int trackIndex) {
    ...
    for (int t = startT; t <= endT; ++t) {
        int div = audioProcessor.trackDivisions[t];
        double ticksPerStep = ticksPerBeat / (double)div;
        int totalSteps = div * audioProcessor.timeSigNumerator.load() * audioProcessor.globalBarCount;

        for (int s = 0; s < totalSteps; ++s) {
            int vel = audioProcessor.drumPattern[t][s];
            if (vel > 0) {
                double start = s * ticksPerStep;
